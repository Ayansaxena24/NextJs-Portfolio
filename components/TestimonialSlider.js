      ...
      {testimonialSlider.map((person, index) => {
        return (
          <SwiperSlide key={index}>
            <div className="flex flex-col items-center md:flex-row gap-x-8 lg:h-full px-16">
              ...
              <div className="flex flex-col justify-center">
                ...
                {/* name */}
                <div className="text-sm lg:text-lg">{person.name}</div>
                {/* position */}
                <div className="text-[8px] lg:text-[12px] uppercase font-extralight tracking-widest">{person.position}</div>
              </div>
            </div>
              {/* quote and message */}
              <div className="flex-1 flex flex-col justify-center before:w-[1px] xl:before:bg-white/20 xl:before:absolute xl:before:left-0 xl:before:h-[200px] relative xl:pl-20">
              <div className="mb-4">
                <FaQuoteLeft className="text-xl lg:text-4xl xl:text-6xl text-white/20 mx-auto md:mx-0" />
              </div>
              {/* message */}
              <div className="text-sm xl:text-lg text-center md:text-left h-[80px] lg:h-full overflow-scroll lg:overflow-auto">
                {person.message}
              </div>
            </div>
            </div>
